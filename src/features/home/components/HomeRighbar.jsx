  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center gap-3 mb-6 relative">
        <img src={user.profile.avatar || "https://randomuser.me/api/portraits/men/32.jpg"} 
              className="w-12 h-12 rounded-full object-cover" 
              alt="avatar" />
        <div>
          <div className="font-semibold text-gray-900 leading-tight">{user.username}</div>
          <div className="text-base text-gray-500">{`${user.profile.lastname || ""} ${user.profile.firstname || ""}`}</div>
        </div>
        <a href="#" className="absolute right-0 top-1/2 -translate-y-1/2 text-blue-500 hover:text-blue-700 p-2"
            onClick={handleLogout}><FiLogOut size={20} /></a>
      </div>
      {/* Gợi ý cho bạn */}
